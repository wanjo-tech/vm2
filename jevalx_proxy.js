...
let jevalx_raw = (js,ctxx,js_opts,ctx_opts={breakOnSigint:true})=>[ctxx,vm.createScript(js,js_opts).runInContext(ctxx,ctx_opts)];
  ...
  try{
    rst = await new Promise((resolve,reject)=>{try{
      ...
      if (ctx && vm.isContext(ctx)) [ctxx,[_console]] = jevalx_raw(S_SESSION,ctx);
        ...
        //TODO eval...hard to write ;)
        if(!ctx) ctx = new X;
        ctxx = vm.createContext(new Proxy(new X,{
          get(target, prop, receiver) {
          switch(prop){
            case 'eval':return undefined;
            ...
            //case 'Symbol(nodejs.util.inspect.custom)':return undefined;
            //case 'ArrayBuffer':return undefined;
            //case 'Uint8Array':return undefined;
            //case 'Int8Array':return undefined;
            case 'globalThis':return ctx;//TODO...
            default:
//console.error('prop',prop, prop in globalThis);
if (typeof prop!='string') return undefined;
              if (ctx[prop]){ return ctx[prop]; }
return undefined;
          }
          throw {message:'!'+prop}
        }}));
      ...
      eval(S_ENTER);
      //let promise=jevalx_raw(`(async()=>await(async(z)=>{while(z&&((z instanceof Promise)&&(z=await z)||(typeof z=='function')&&(z=z())));return z})(eval(${jss})))()`,ctxx,{filename:call_id})[1];//OK
      let promise0=jevalx_raw(js,ctxx,{filename:call_id})[1];
      ctx.promise0 = promise0;
//console.error('promise0',promise0);
      let promise=jevalx_raw(`(async()=>await(async(z)=>{while(z&&((z instanceof Promise)&&(z=await z)||(typeof z=='function')&&(z=z())));return z})(promise0))()`,ctxx,{filename:call_id})[1];//OK
