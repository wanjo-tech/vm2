...
let jevalx= async(js,ctx,options={})=>{
  ...
  try{
    rst = await new Promise((resolve,reject)=>{try{
      let _console,_Promise;
      timers.setTimeout(()=>{reject({message:'TimeoutX',code:'TIMEOUTX',js})},timeout+11);//Q7x
      if (ctx && vm.isContext(ctx)) [ctxx,[_console,_Promise]] = jevalx_raw(S_SESSION,ctx);
      else {
        ctxx = vm.createContext(new X,{microtaskMode});
        [ctxx,[_console,_Promise]] = jevalx_raw(S_SETUP,ctxx);
        _console.error = console.log; _console.error = console.error;
        ...
        if (ctx) Object.assign(ctxx,ctxx_proxy);
      }
      eval(S_ENTER);
